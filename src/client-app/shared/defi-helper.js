var DeFiHelper = {
    ...
    ChainName: 'BNB',
    CurrencyName: 'BNB',
    RpcUrls: 'https://quiet-weathered-voice.bsc.quiknode.pro/d3e2ded180c61cba24bcc1f22baad17a0ba707bd/',
    CurrentAddress: '',
    CurrentSponsorId: '',
    BalanceInEth: 0.00,
    ContractAddress: '',
    ...
    CheckNetWork: async function fn() {
        ...
        try {
            ...
            console.log('DAPP Notification',
                'Wrong network, please chain your network to BSC. Current Chain: '
                + currentChain)
                ...
                try {
                    await ethereum.request({
                        ...
                        params: [{
                            ...
                            rpcUrls: [DeFiHelper.RpcUrls],
                            nativeCurrency: {
                                name: DeFiHelper.CurrencyName,
                    ...
                    // handle "add" error
                    console.log('DAPP Notification', addError.message)
                    return false
                }
            }

            console.log('DAPP Notification', switchError.message)
            return false
        }
    },
        ...
        function registerEvents() {
            ...
            $('#wallet-connect-metamask').on('click', async function (e) {
                ...
                if (!DeFiHelper.HasInstalledMetaMask()) {
                    if (be.isDevice()) {
                        let referralHidden = $('.dapp_ref_code').val();

                        var url = 'https://metamask.app.link/dapp/fbsdefi.com/reflink/' + referralHidden;
            ...
            //$('#wallet-connect-walletconnect').on('click', async function (e) {
            //    e.preventDefault();
            //    console.log("comming");
        ...
        async function handleRequestAccounts() {
            try {
                ...
                if (accounts && accounts.length > 0) {
                    ...
                    /*DeFiHelper.PostAsync(`/DAPP/InitializeTransactionProgress/${action}`, data);*/
                    return accounts
                }
            } catch (e) {
                //ShowConnectButton()
    ...
    DAppConnect: async function DAppConnect(address) {
        ...
        let result = await DeFiHelper.GetAsync(`/SavingDefi/DAppConnect?address=${address}&referral=${referralHidden}`);
